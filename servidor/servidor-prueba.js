...
app.post("/api/conversacion/:userId", async (req, res, next) => {
  ...
  if (!conversaciones[userId]) {
    conversaciones[userId] = [{ rol: 'Asistente', contenido: "Saluda cordialmente con una bienvenida una sola vez en toda la conversación, no vuelvas a saludar ante cada respuesta al mismo usuario. Solamente puedes contestar sobre esto que sigue, en caso que no puedas responder di que no estás preparado para contestar sobre ese tema. " + 
